        ...
        while not finished:
            ...
            if option.upper() == 'Q':
                ...
                if self.wallet.public_key is None:
                    ...
                    txn = self.block_chain.add_transaction(self.wallet.public_key, recipient, amount, signature)
                    if txn is not None:
                        print('Transaction successfully added : ' + str(txn))
                    else:
                        print('WARN: Failed to add transaction')
            elif option == '2':
                if len(self.block_chain.open_txns) == 0:
                    ...
                    if mined_block is not None:
                        ...
                        print('Block successfully mined : ' + str(mined_block))
                    else:
                        print('WARN: Failed to mine block')
            elif option == '3':
                if bc_len == 0:
                ...
                if self.wallet.load_keys():
                    ...
                    print('Wallet keys successfully loaded')
                else:
                    print('WARN: Failed to load wallet keys')
            elif option == '8':
                self.wallet.create_keys()
                self.init_block_chain()
            elif option == '9':
                if self.wallet.save_keys():
                    print('Wallet keys successfully saved')
                else:
                    print('WARN: Failed to save wallet keys')
            elif option.upper() == 'V':
